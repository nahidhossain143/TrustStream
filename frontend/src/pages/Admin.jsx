...
function PipelineStep({ icon, label, sublabel, status, isDark }) {
  return (
    ...
    >
      <div
        className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 border transition-all duration-500 ${
          status === "done"
...
function ResultRow({ label, value, mono, link, isDark }) {
  return (
    ...
    >
      <span
        className={`text-[10px] uppercase tracking-widest font-mono w-28 flex-shrink-0 pt-0.5 ${
      ...
      {link ? (
        ...
        <span
          className={`text-[11px] break-all ${mono ? "font-mono" : ""} ${
...
export default function Admin() {
  ...
  const startPolling = (videoId) => {
    pollRef.current = setInterval(async () => {
      try {
        ...
        if (ipfsDone && chainDone && forensicDone) {
  ...
  const forensicRiskScore =
    bgStatus?.forensicRiskScore ?? result?.forensicRiskScore ?? null;
  ...
  const allDone = pipelineSteps.every((step) => getPipelineStatus(step.key) === "done");
          ...
          {(stage === "processing" || stage === "done") && (
            <div
              className={`border-t px-6 py-5 space-y-4 ${
                isDark ? "border-white/6 bg-white/1" : "border-neutral-100 bg-neutral-50/50"
              }`}
            >
              ...
              <div className={`rounded-xl border overflow-hidden ${isDark ? "border-neutral-800" : "border-neutral-200"}`}>
                <div className={`${isDark ? "bg-neutral-800/60" : "bg-neutral-100"} px-4 py-1`}>
                  {[
                    ...
                    { label: "Risk Label", value: forensicLabel || "pending" },
                    {
                      ...
                      value:
                        forensicRiskScore != null
                          ? `${Math.round(forensicRiskScore * 100)}%`
                          : "pending",
                    },
                    ...
                    {
                      label: "Blockchain",
                      value: bgStatus?.blockchainStatus || result.blockchainStatus || "pending",
                    },
                  ...
                  {(bgStatus?.forensicReportCid || result.forensicReportCid) && (
                    <ResultRow
                      label="Forensic CID"
                      value={bgStatus?.forensicReportCid || result.forensicReportCid}
                      mono
                      link={
                        bgStatus?.forensicReportUrl ||
                        result.forensicReportUrl ||
                        `https://gateway.pinata.cloud/ipfs/${bgStatus?.forensicReportCid || result.forensicReportCid}`
                      }
                      isDark={isDark}
                    />
                  )}
                    ...
                    {forensicNotes.slice(0, 3).map((note) => (
                      <div
                        ...
                        className={`text-xs rounded-lg px-3 py-2 border ${
                          isDark
                            ? "text-neutral-300 bg-neutral-950/70 border-neutral-800"

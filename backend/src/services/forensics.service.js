...
const analyzeCompressionAndMetadata = ({ sourceInfo, packetInfo, frameInfo }) => {
  ...
  const compressionScore    = clamp(bitrateVariationScore * 0.45 + gopIrregularityScore * 0.35 + fpsMismatchScore * 0.1 + durationScore * 0.1);
...
const analyzeTemporalConsistency = ({ frameBuffer, totalSegments, segmentDurationSeconds }) => {
  ...
  const flickers = [], boundaryDiffs = [], textureShifts = [], motionDiffs = [], sceneCutTimes = [];
  ...
  for (let i = 1; i < rawFrames.length; i++) {
    ...
    textureShifts.push(textureShift); motionDiffs.push(motionDiff);
    segmentBuckets[segmentIndex].temporalSamples.push(clamp(motionDiff * 0.45 + flicker * 0.2 + boundaryDiff * 0.2 + textureShift * 0.15));
  ...
  const temporalAnomalyScore    = clamp(flickerScore * 0.3 + boundaryInstabilityScore * 0.3 + textureShiftScore * 0.2 + motionDisruptionScore * 0.2);
